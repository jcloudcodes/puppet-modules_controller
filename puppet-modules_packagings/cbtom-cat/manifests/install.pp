class tom_cat::install (
  ...
  if $facts['kernel'] == 'Linux' {
    ...
    exec { 'cleanup_tomcat_temp_windows':
      command     => "${windows_powershell} -NoProfile -ExecutionPolicy Bypass -Command \"\$paths = @('${windows_corretto_zip}','${windows_temp}/apache-tomcat-${tom_version}-windows-x64.zip','${windows_extract_root}','${windows_temp}/tomcat-extract'); foreach (\$path in \$paths) { if (Test-Path \$path) { Remove-Item -Path \$path -Recurse -Force } }\"",
      onlyif      => "${windows_powershell} -NoProfile -Command \"\$paths = @('${windows_corretto_zip}','${windows_temp}/apache-tomcat-${tom_version}-windows-x64.zip','${windows_extract_root}','${windows_temp}/tomcat-extract'); foreach (\$path in \$paths) { if (Test-Path \$path) { exit 0 } }; exit 1\"",
      timeout     => 600,
